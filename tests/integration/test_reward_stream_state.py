...
class _State:
    ...
    def remove_receiver(self, receiver: str, msg_sender: str, timestamp: int):
        ...
        if amount > 0:
            ...
            pass
        self._lifetime_earnings[receiver] = (
            self._reward_paid[receiver] - self._reward_start[receiver]
        )
    ...
    def get_reward(self, msg_sender: str, timestamp: int):
        ...
        if amount > 0:
            ...
            self._lifetime_earnings[msg_sender] = (
                self._reward_paid[msg_sender] - self._reward_start[msg_sender]
            )
...
class StateMachine:
    ...
    def teardown(self):
        chain.mine(1000, timestamp=self.state.period_finish)
        for acct, val in self.state._lifetime_earnings.items():
            ...
            assert self.reward_token.balanceOf(acct) == val
